...
public class PlayerControl : MonoBehaviour
{
    ...
    [SerializeField]
    private Transform stepRayPoint;
    ...
    [Header("Stair step settings")]
    public float lowRayOffSet;
    public float highRayOffSet;
    ...
    private void StepClimb()
    {
        ...
        //float rayLength = 0.1f * Vector3.Dot(transform.forward, rb.linearVelocity);
        float rayLength = stepLenght;
        Vector3 rayDir = (stepRayPoint.forward * moveDir.y + stepRayPoint.right*moveDir.x).normalized;
        bool goUp = false;
        //Forward
        if (Physics.Raycast(stepRayPoint.position + new Vector3(0,lowRayOffSet,0), rayDir, rayLength))
        {
            if(!Physics.Raycast(stepRayPoint.position + new Vector3(0, highRayOffSet, 0), rayDir, rayLength ))
            {
                goUp = true;
            }
        ...
        //45
        if (Physics.Raycast(stepRayPoint.position + new Vector3(0, lowRayOffSet, 0), Quaternion.Euler(0, 45, 0) * rayDir, rayLength))
        {
            if (!Physics.Raycast(stepRayPoint.position + new Vector3(0, highRayOffSet, 0), Quaternion.Euler(0, 45, 0) * rayDir, rayLength))
            {
                goUp = true;
            }
        }
        ...
        if (goUp)
        {
            rb.position = Vector3.Lerp(rb.position, rb.position + new Vector3(0, stepSmooth, 0f), stepSpeed); 
        }
    }
